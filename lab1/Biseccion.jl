function biseccion(f :: Function, a:: Number, b:: Number, error :: AbstractFloat = 1e-5, iter::Int64 = 100) #parametros: ecuacion no linel, el intervalo, la tolerancia y la cantidad maxima de iteraciones 
    MaximoError = (b-a)/2 # Forma del error en el metodo de biseccion (Xnueva - Xanterior)/Xnueva

    n = 0 # cantidad de iteraciones

    while MaximoError > error || n > iter  # ciclo que se detiene al momemento de que la tolerancia supere al error maximo o se supere el numero de iteraciones
